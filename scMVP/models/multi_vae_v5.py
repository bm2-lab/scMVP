...
class Multi_VAE_v5(nn.Module):
    ...
    def __init__(
        ...
        if self.mode == "vae":
            ...
            self.RNA_ATAC_decoder = Multi_Decoder_nb_log_peak(
                n_latent,
                RNA_input,
                ATAC_input,
                n_cat_list=[n_batch],
    ...
    def forward(self, x_rna, x_atac, local_l_mean, local_l_var, batch_index=None, y=None):
        ...
        # likelihood
        reconst_loss_rna = 3*self.get_reconstruction_loss(x[0], p_rna_rate, p_rna_r, p_rna_dropout)
